...
public class DlgCekinMobileJKN extends javax.swing.JDialog {    
    ...
    private void initComponents() {
        ...
        keypadBackspace.getAccessibleContext().setAccessibleName("Backspace");
    ...
    private void inputKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_inputKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
            ...
            if (query.cariBooleanSmc("select * from referensi_mobilejkn_bpjs where nomorkartu = ? and tanggalperiksa = current_date() and status != 'Batal'", input.getText())) {
                form.tampil(input.getText());
                form.setSize(this.getWidth(), this.getHeight());
                form.setLocationRelativeTo(jPanel1);
                form.setVisible(true);
                this.dispose();
                this.setCursor(Cursor.getDefaultCursor());
            } else if (query.cariBooleanSmc("select * from referensi_mobilejkn_bpjs where norm = ? and tanggalperiksa = current_date() and status != 'Batal'", input.getText())) {
                form.tampil(query.cariIsi("select nomorkartu from referensi_mobilejkn_bpjs where norm = ? and tanggalperiksa = current_date() and status != 'Batal'", input.getText()));
                form.setSize(this.getWidth(), this.getHeight());
                form.setLocationRelativeTo(jPanel1);
                form.setVisible(true);
                this.dispose();
    ...
    private void keypadBackspaceActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_keypadBackspaceActionPerformed
        if (input.getText().length() > 0) {
            ...
            back.deleteCharAt(input.getText().length() - 1);
